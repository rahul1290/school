<div class="fixed bottom-0 left-0 w-full bg-gradient-to-r from-orange-500 to-red-500 border-t border-orange-600 shadow-xl z-50 md:hidden pb-safe">
    <div class="flex justify-around items-center h-16">
        ...
        <!-- About -->
        <a href="{{ url('/about-us') }}" class="flex flex-col items-center justify-center w-full h-full transition-colors group {{ request()->is('about-us') || request()->is('aboutus/*') ? 'text-white font-bold' : 'text-orange-100 hover:text-white' }}">
            <svg class="w-6 h-6 mb-1 group-hover:-translate-y-1 transition-transform {{ request()->is('about-us') || request()->is('aboutus/*') ? 'scale-110' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Admission -->
        <a href="{{ url('/admission') }}" class="flex flex-col items-center justify-center w-full h-full transition-colors group {{ request()->is('admission') ? 'text-white font-bold' : 'text-orange-100 hover:text-white' }}">
            <svg class="w-6 h-6 mb-1 group-hover:-translate-y-1 transition-transform {{ request()->is('admission') ? 'scale-110' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
            </svg>
            <span class="text-[10px] tracking-wide uppercase">Admission</span>
        </a>
